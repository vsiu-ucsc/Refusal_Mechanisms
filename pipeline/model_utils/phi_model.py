...
class PhiModel(ModelBase):
    ...
    def _get_post_attn_modules(self):
        return [layer.post_attention_layernorm for layer in self.model.model.layers]
    
    def compute_f_local(self, layer_idx: int) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        """
        Compute F_local = W_down W_down^T + W_O W_O^T for Phi3.
        Returns eigenvalues (descending), eigenvectors, F_local.
        """
        mlp_module = self.model_mlp_modules[layer_idx]
        attn_module = self.model_attn_modules[layer_idx]
